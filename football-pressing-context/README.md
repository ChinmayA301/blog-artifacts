...
## Additional study scripts

The `code/` folder contains two separate companion studies used by the updated
Ronaldo/Portugal analysis. These are independent from the StatsBomb pressing
study generated by `generate_figures.py`.

| File | What it does |
|---|---|
| `code/physical_output_context_study.py` | Builds a simple public aggregate physical-output comparison for selected World Cup forwards. |
| `code/screenshot_trap_study_template.py` | Creates a manual-coding template for testing viral screenshot claims against repeated comparable situations. |

These scripts write generated tables and figures to `code/outputs/`, which is
not committed.
...
python code/physical_output_context_study.py
python code/screenshot_trap_study_template.py
```

The first run downloads and caches StatsBomb open-data JSON. Later runs reuse
the local cache.

The companion physical-output and screenshot-trap scripts are independent of the
StatsBomb event-data run.
